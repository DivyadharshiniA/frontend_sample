...
const UpdateUser = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://backend-sample-2.onrender.com/api/user/${id}`) // Corrected endpoint
      .then((res) => {
        setName(res.data.name);
        setEmail(res.data.email);
        setAddress(res.data.address);
  ...
  const updateUser = (e) => {
    e.preventDefault();
    axios
      .put(`https://backend-sample-2.onrender.com/api/user/update/${id}`, { name, email, address }) // Corrected endpoint
      .then((result) => {
        alert("User updated successfully");
        console.log(result.data);
